...
const int NR = 1201, NZ = 81, NT = 100000;
const double dR1 = 0.01, dZ1 = 0.01, dR2 = 0.2, dZ2 = 0.2; 
const double R_inj = 8.0, Z_inj = 1.0, Rc = 10.0, Zc = 0.3, ratio_threshold = 1e-6; 
...
void initialize_H(double **ndis_H, double R[NR], double Z[NZ]){
    ...
    for (int i = 0; i < NR; i++) {
        for (int j = 0; j < NZ; j++) {
            ...
            ndis_H2[i][j] = ratio * 2200. / 4. / 45. * exp(-12./R[i] - R[i] / 1.5) / pow(cosh(Z[j] / 2. / 0.045), 2);
            ndis_HI[i][j] = ratio * 53. / 4. / 85. * exp(-4./R[i] - R[i] / 7) / pow(cosh(Z[j] / 2. / 0.085), 2);
            ndis_HII[i][j] = 0.00015 * (1. + 3.7*log(1 + r0 / 20.) / (r0/20.) - 1.0277);
            ndis_H[i][j] = ndis_H2[i][j] + ndis_HI[i][j] + ndis_HII[i][j];
            // ndis_H[i][j] = ndis_HII[i][j];
...
void solve_diffusion_equation(double **ndis_C, double **ndis_B, double **ndis_H, double **ndis_C_temp, double NC[NT], double NB[NT], double D, double dT, double R[NR], double Z[NZ], double dR[NR], double dZ[NZ]) {
    ...
    for (int t = 0; t < NT; t++) {
        ...
        for (int j = 1; j < NR - 1; j++) {
            for (int k = 1; k < NZ - 1; k++) {
                // double R = j * dR;
                double d2n_dR2_C = (2.0 / (dR[j] * (dR[j] + dR[j+1]))) * ndis_C[j - 1][k] - (2.0 / (dR[j] * dR[j+1])) * ndis_C[j][k] + (2.0 / (dR[j+1] * (dR[j] + dR[j+1]))) * ndis_C[j + 1][k];
                double dn_dR_C = (ndis_C[j][k] - ndis_C[j - 1][k]) / dR[j] / 2.0 + (ndis_C[j + 1][k] - ndis_C[j][k]) / dR[j+1] / 2.0;
                double d2n_dZ2_C = (2.0 / (dZ[k] * (dZ[k] + dZ[k+1]))) * ndis_C[j][k - 1] - (2.0 / (dZ[k] * dZ[k+1])) * ndis_C[j][k] + (2.0 / (dZ[k+1] * (dZ[k] + dZ[k+1]))) * ndis_C[j][k + 1];
                double decay_C = ndis_C[j][k] * ndis_H[j][k] * sigma_pC * c * 1e-27 * dT * yr2s;
                ndis_C_temp[j][k] = ndis_C[j][k] + dT * D * (d2n_dR2_C + dn_dR_C / R[j] + d2n_dZ2_C) - decay_C;  // Carbon number density distribution at this time step

                double d2n_dR2_B = (2.0 / (dR[j] * (dR[j] + dR[j+1]))) * ndis_B[j - 1][k] - (2.0 / (dR[j] * dR[j+1])) * ndis_B[j][k] + (2.0 / (dR[j+1] * (dR[j] + dR[j+1]))) * ndis_B[j + 1][k];
                double dn_dR_B = (ndis_B[j][k] - ndis_B[j - 1][k]) / dR[j] / 2.0 + (ndis_B[j + 1][k] - ndis_B[j][k]) / dR[j+1] / 2.0;

                double d2n_dZ2_B = (2.0 / (dZ[k] * (dZ[k] + dZ[k+1]))) * ndis_B[j][k - 1] - (2.0 / (dZ[k] * dZ[k+1])) * ndis_B[j][k] + (2.0 / (dZ[k+1] * (dZ[k] + dZ[k+1]))) * ndis_B[j][k + 1];
                double decay_B = ndis_B[j][k] * ndis_H[j][k] * sigma_pB * c * 1e-27 * dT * yr2s;
                ndis_B[j][k] = ndis_B[j][k] + dT * D * (d2n_dR2_B + dn_dR_B / R[j] + d2n_dZ2_B) - decay_B + decay_C; // Boron number density distribution at this time step
            }
        }
        ...
        for (int j = 0; j < NR; j++) {
            for (int k = 0; k < NZ; k++) {
                ...
                if (R[j] <= R_inj && Z[k] <= Z_inj) {
                    ndis_C[j][k] = 1.0 * dT + ndis_C_temp[j][k];  // ndis_C is the Carbon density at this time step after injection
                } 
                else {
                    ndis_C[j][k] = ndis_C_temp[j][k];
                }
                if (j < NR - 1){
                    NC[t] += (ndis_C_temp[j][k] + ndis_C_temp[j + 1][k] + ndis_C_temp[j][k+1] + ndis_C_temp[j+1][k+1]) / 4.0 * (R[j] + R[j+1]) / 2.0 * dR[j] * dZ[k];
                    NB[t] += (ndis_B[j][k] + ndis_B[j + 1][k] + ndis_B[j][k+1] + ndis_B[j+1][k+1]) / 4.0 * (R[j] + R[j+1]) / 2.0 * dR[j] * dZ[k];              
                }
            }
        }
        if (t > 0){            
...
// Grid generation
    ...
    for (int e = 0; e < 1; e++) {
        ...
        apply_boundary_conditions(ndis_B);
        solve_diffusion_equation(ndis_C, ndis_B, ndis_H, temp, NC, NB, D, dT, R, Z, dR, dZ);

        snprintf(filename1, sizeof(filename1), "NC_R%d_Z%d_D%dGeV_dR2_%.2f.txt", (int)R[NR-1], (int)Z[NZ-1], (int)E[e], dR2);
        snprintf(filename2, sizeof(filename2), "NB_R%d_Z%d_D%dGeV_dR2_%.2f.txt", (int)R[NR-1], (int)Z[NZ-1], (int)E[e], dR2);
        snprintf(filename3, sizeof(filename3), "ndisC_R%d_Z%d_D%dGeV_dR2_%.2f.txt", (int)R[NR-1], (int)Z[NZ-1], (int)E[e], dR2);
        snprintf(filename4, sizeof(filename4), "ndisB_R%d_Z%d_D%dGeV_dR2_%.2f.txt", (int)R[NR-1], (int)Z[NZ-1], (int)E[e], dR2);
