...
const int NR = 1381, NZ = 499, NT = 1000000;
const double dR1 = 0.01, dZ1 = 0.01, dR2 = 0.5, dZ2 = 0.5; 
const double R_inj = 8.0, Z_inj = 1.0, Rc = 10.0, Zc = 1.0, ratio_threshold = 1e-6; 
    ...
    for (int t = 0; t < NT; t++) {
        ...
        for (int j = 0; j < NR; j++) {
            for (int k = 0; k < NZ; k++) {
                ...
                // if (j < NR - 1){
                NC[t] = ndis_C_temp[800][0];
                NB[t] = ndis_B[800][0];              
                // }
            }
        }
        if (t > 0){            
            double ratio_C = (NC[t] - NC[t - 1]) / NC[t - 1];
            double ratio_B = (NB[t] - NB[t - 1]) / NB[t - 1];
...
// Grid generation
    ...
    for (int e = 3; e < 4; e++) {
        ...
        snprintf(filename1, sizeof(filename1), "NC_R%d_Z%d_D%dGeV.txt", (int)R[NR-1], (int)Z[NZ-1], (int)E[e]);
        snprintf(filename2, sizeof(filename2), "NB_R%d_Z%d_D%dGeV.txt", (int)R[NR-1], (int)Z[NZ-1], (int)E[e]);
        snprintf(filename3, sizeof(filename3), "ndisC_R%d_Z%d_D%dGeV.txt", (int)R[NR-1], (int)Z[NZ-1], (int)E[e]);
        snprintf(filename4, sizeof(filename4), "ndisB_R%d_Z%d_D%dGeV.txt", (int)R[NR-1], (int)Z[NZ-1], (int)E[e]);
