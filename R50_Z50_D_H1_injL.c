...
const int NR = 501, NZ = 501, NT = 2000000;
const double dR = 0.1, dZ = 0.1; 
const double R_inner_disk = 5.0, Z_disk = 0.5, R_outer_disk = 30.0, ratio_threshold = 1e-7; 
    ...
    for (int i = 0; i < NR; i++) {
        ...
        for (int j = 0; j < NZ; j++) {
            ...
            if (R_temp <= R_inner_disk) {
                ...
                ndis_H[i][j] = 1*(ndis_H2[i][j] + ndis_HI[i][j] + ndis_HII[i][j]);
...
void solve_diffusion_equation(double **ndis_C, double **ndis_B, double **ndis_H, double **temp, double NC[NT], double NB[NT], double D, double dT) {
    for (int t = 0; t < NT; t++) {
...
int main(void) {
    ...
    double E[7] = {1, 5, 10, 50, 100, 500, 1000}, D, dT;
    ...
    for (int e = 0; e < 7; e++) {
        ...
        dT = dR * dR / D / 10.0; // dT <= (dR^2+dZ^2) / D
        // Initialize particle density and apply boundary conditions
        initialize_C(ndis_C);
        initialize_B(ndis_B);
        apply_boundary_conditions(ndis_C);
        apply_boundary_conditions(ndis_B);
        solve_diffusion_equation(ndis_C, ndis_B, ndis_H, temp, NC, NB, D, dT);
        snprintf(filename3, sizeof(filename3), "ndisC_R%d_Z%d_D%dGeV_H1_disk5.txt", (int)R_max, (int)Z_max, (int)E[e]);
        snprintf(filename4, sizeof(filename4), "ndisB_R%d_Z%d_D%dGeV_H1_disk5.txt", (int)R_max, (int)Z_max, (int)E[e]);
    ...
    // snprintf(filename1, sizeof(filename1), "NC_t%d_R%d_Z%d_D50GeV_H1.txt", NT, (int)R_max, (int)Z_max);
    // snprintf(filename2, sizeof(filename2), "NB_t%d_R%d_Z%d_D50GeV_H1.txt", NT, (int)R_max, (int)Z_max);

    // snprintf(filename5, sizeof(filename5), "ndisH_t%d_R%d_Z%d_D50GeV_H1.txt", NT, (int)R_max, (int)Z_max);
